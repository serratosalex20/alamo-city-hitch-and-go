...
import type { BookingStatus, DepositStatus } from "@/types/models";
...
export function AdminBookingActions(props: Props) {
  ...
  async function act(action: string) {
    ...
    try {
      ...
      setMessage({ ok: true, text: "Booking updated." });
      router.refresh();
    } catch (error) {
  ...
  return (
    <section className="space-y-5 bg-surface-container-low p-6 ghost-border" aria-labelledby="owner-actions">
      ...
      {props.status === "active" && <button disabled={busy !== null} onClick={() => act("mark_returned")} className={actionClass}>Mark Returned</button>}
